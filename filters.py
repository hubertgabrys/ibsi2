...
class Mean:
    def __init__(self, padding_type, support=15, dimensionality="3D"):
...
class Laws:
    ...
    def __init__(self, response_map, padding_type, dimensionality="3D", rotation_invariance=False, pooling=None):
    ...
    def filter(self, img):
        ...
        if self.rotation_invariance:
            ...
            if self.pooling == "avg":
                ...
                return final_image / 24  # shouldn't it rather be len(response_maps) * 8 = 48?
    ...
    def get_energy_map(self, img, distance):
        if self.dimensionality == "2D":
            filt_mat = np.ones([2 * distance + 1, 2 * distance + 1])
            filt_mat = filt_mat / np.prod(filt_mat.shape)
            energy_map = np.nan * np.ones(img.shape)
            for i in range(img.shape[2]):
                energy_map[:, :, i] = ndi.convolve(input=np.abs(img[:, :, i]), weights=filt_mat, mode='reflect')
        elif self.dimensionality == "3D":
            filt_mat = np.ones([2 * distance + 1, 2 * distance + 1, 2 * distance + 1])
            filt_mat = filt_mat / np.prod(filt_mat.shape)
...
class Gabor:
    ...
    def __init__(self, padding_type, res_mm, sigma_mm, lambda_mm, gamma, theta, rotation_invariance=False, orthogonal_planes=False):
        self.rotation_invariance = rotation_invariance
        self.res_mm = res_mm
        self.theta = theta
        self.gamma = gamma
    ...
    def _filter(self, img, theta, plane2d=(0, 1)):
        ...
        if z_axis != 2:
            img = np.swapaxes(img.copy(), 2, z_axis)

        # calculate the kernel size
        kernel_size = [size + 1 if size % 2 == 0 else size for size in [img.shape[idx] for idx in plane2d]]

        # calculate the filtered image
        filtered_img = np.ones(img.shape)
        for i in range(img.shape[2]):
            kernel_real = getGaborKernel(ksize=kernel_size, sigma=sigma, theta=theta, lambd=lambd,
                                         gamma=self.gamma, psi=0)
            kernel_imag = getGaborKernel(ksize=kernel_size, sigma=sigma, theta=theta, lambd=lambd,
                                         gamma=self.gamma, psi=np.pi / 2)
            filt_real = ndi.convolve(img[:, :, i], kernel_real, mode=self.padding_type)
            filt_imag = ndi.convolve(img[:, :, i], kernel_imag, mode=self.padding_type)
            filtered_img[:, :, i] = np.sqrt(filt_real ** 2 + filt_imag ** 2)
    ...
    def filter(self, img):
        if self.rotation_invariance:
            ...
            with tqdm_joblib(tqdm(desc="Gabor planes", total=len(parameter_space))):
                filtered_images = Parallel(n_jobs=-2)(delayed(self._filter)(img, delta_theta, plane2d) for delta_theta, plane2d in parameter_space)

            filtered_img = reduce(lambda a, b: a + b, filtered_images)
            filtered_img = filtered_img / (len(thetas) * len(planes))
        else:
